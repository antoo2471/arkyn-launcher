...
export default class Launch extends EventEmitter {
	...
	async Launch(opt: LaunchOPTS): Promise<ChildProcess | void> {
		...
		if (!this.options.authenticator) {
			this.emit("error", { error: "Authenticator not found" });
			return;
		}
	...
	async start(): Promise<ChildProcess | void> {
