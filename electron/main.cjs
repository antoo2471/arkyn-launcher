...
const richPresence = require('discord-rich-presence')('1470506574492467417'); 
...
function releaseMainWindowIfAllowed() {
    ...
    const allowed = updaterState.status === 'up-to-date' || updaterState.status === 'disabled' || updaterState.status === 'idle' || updaterState.status === 'checking';
    if (!allowed) return;
...
function installDownloadedUpdate() {
    ...
    setUpdaterState({
        enabled: true,
        status: 'installing',
        message: 'Installation de la mise a jour en cours...'
    });
    ...
    setTimeout(() => {
        try {
            ...
            setUpdaterState({
                enabled: true,
                status: 'error',
                message: error?.message || 'Installation de la mise a jour impossible'
            });
        }
    }, 200);
}
...
ipcMain.handle('launch-game', async (event, options) => {
    ...
    try {
        ...
        const authenticator = await authManager.refreshIfNeeded();
...
ipcMain.handle('microsoft-session', async () => {
    try {
        // Return the current active account since volatile accounts are removed
        const account = await authManager.loadAccount();
        if (!account) return { success: false };
        return { success: true, result: authManager.getPublicAccount(account), rememberMe: true };
    } catch (error) {
...
ipcMain.handle('microsoft-select-account', async (event, payload = {}) => {
    try {
        ...
        volatileAccount = null; // Clear volatile account reference
...
ipcMain.handle('microsoft-login', async (event, payload = {}) => {
    try {
        // Always persist accounts since remember me option was removed from UI
        const account = await authManager.login({ persist: true });
        volatileAccount = null; // No more volatile accounts
        return { success: true, result: authManager.getPublicAccount(account) };
    } catch (error) {
        if (error instanceof AuthManagerError) {
